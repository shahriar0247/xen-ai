...
def create_db():
    global conn
    global cursor
    conn = sqlite3.connect("database/programlist.db")
    cursor = conn.cursor()
...
def list_main_programs_dir():
    ...
    main_folder_applist_with_path = list_all_files(
        os.environ['WINDIR'] +
    ...
    for a in main_folder_applist_with_path:
        ...
        cursor.execute("INSERT INTO program_list VALUES ('" + app_name +
                       "','" + a + "')")
        conn.commit()
...
def list_user_programs_dir():
    ...
    user_folder_applist_with_path = list_all_files(
...
def start_program(program_name):
    ...
    program_requested, programs_with_same_name = specify_program(
        programs_with_same_name)
